...
SEASON = 2024
OUT_PATH = "data/raw/race_laps_2024.csv"
...
for _, row in tqdm(schedule.iterrows(), total=len(schedule), desc="Fetching Race sessions"):
    event_name = row["EventName"]

    try:
        session = get_session(SEASON, event_name, "R")
        session.load()
        ...
        print(f"[SKIPPED] {event_name} (Race) -> {e}")

race_df = pd.concat(all_rows, ignore_index=True)
race_df.to_csv(OUT_PATH, index=False)

print(f"\n? Saved: {OUT_PATH}")
print("Rows:", len(race_df))
